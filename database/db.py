    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS admin_confirm_polls (
                poll_id TEXT PRIMARY KEY, question_id INTEGER NOT NULL
            );
            ...
            CREATE TABLE IF NOT EXISTS batch_scores (
                ...
                first_answer_at TEXT, submitted_at TEXT,
...
async def confirm_question_in_db(q_id, correct_option, explanation):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE question_queue SET correct_option=?,explanation=?,status='confirmed' WHERE id=?",
...
async def mark_question_needs_review(q_id, reason="AI failed"):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE question_queue SET status='needs_review',explanation=? WHERE id=?", (reason, q_id))
        await db.commit()
...
async def upsert_score(batch_id, user_id, username, correct, total):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """INSERT INTO batch_scores(batch_id,user_id,username,score,total) VALUES(?,?,?,?,?)
               ON CONFLICT(batch_id,user_id) DO UPDATE SET
               score=score+excluded.score,total=excluded.total,username=excluded.username""",
            (batch_id, user_id, username, 1 if correct else 0, total))
        await db.execute(
            "UPDATE batch_scores SET first_answer_at=? WHERE batch_id=? AND user_id=? AND first_answer_at IS NULL",
...
async def record_submit(batch_id, user_id):
    now = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE batch_scores SET submitted_at=? WHERE batch_id=? AND user_id=?",
                         (now, batch_id, user_id))
        await db.commit()
