...
# ── Smart explanation extractor ──
async def _smart_explanation(solution_text: str, question: str, options: list, correct_idx: int) -> str:
    """
    Extract first meaningful sentence from solution.
    Falls back to AI if extraction fails or result is poor quality.
    """
    if solution_text:
        cleaned = strip_html(solution_text).strip()
        # Try to extract sentence after "Explanation:" keyword
        exp_match = re.search(r'[Ee]xplanation\s*[:\-]\s*(.+?)(?:\n|$)', cleaned)
        if exp_match:
            first_sentence = exp_match.group(1).strip()
        else:
            # Take first sentence of solution
            sentences = re.split(r'(?<=[.!?])\s+', cleaned)
            # Skip lines like "Solution: Correct Answer is X"
            first_sentence = ""
            for s in sentences:
                s = s.strip()
                if (len(s) > 15 and
                        not s.lower().startswith("solution") and
                        not s.lower().startswith("correct answer")):
                    first_sentence = s
                    break

        if 10 <= len(first_sentence) <= 200:
            logger.info("[EXPLANATION] Extracted from solution: %s", first_sentence[:60])
            return first_sentence

    # Fallback — generate with AI
    logger.info("[EXPLANATION] Falling back to AI for: %s", question[:60])
    try:
        return await get_explanation_for_answer(question, options, correct_idx)
    except Exception:
        logger.error("AI explanation fallback failed", exc_info=True)
...
# ── JSON parsing ──
def parse_json_questions(json_content: str):
    """
    Parse questions from JSON format:
    {topic, section_name, questions: [{question, options, correct_answer_number, solution}]}
    """
    try:
    ...
    subject = data.get("topic") or data.get("section_name") or "Quiz"
    ...
    for q in raw_questions:
        text = strip_html(q.get("question", "")).strip()
        options = [strip_html(o.get("text", "")).strip() for o in q.get("options", [])]
        correct_num = q.get("correct_answer_number", 1)
        correct_idx = int(correct_num) - 1  # 1-indexed → 0-indexed
        solution = q.get("solution", "")  # keep raw for smart extraction
        if text and options and len(options) >= 2:
            questions.append({
                "text": text,
                "options": options,
                "correct_index": correct_idx,
                "solution_raw": solution,  # raw for AI/extraction
            })

    logger.info("[JSON PARSE] subject='%s' found=%d", subject, len(questions))
    return questions, subject, len(questions)
...
async def handle_html_upload(msg, user_id, context):
    ...
    if not questions:
        await msg.reply_text("❌ No questions found in this HTML file.")
        return True
...
# ── JSON upload handler ──
async def handle_json_upload(msg, user_id, context):
    doc = msg.document
    if not doc or not doc.file_name.endswith(".json"): return False
    await msg.reply_text("📋 Reading JSON file...")
    try:
        file = await context.bot.get_file(doc.file_id)
        buf = io.BytesIO()
        await file.download_to_memory(buf)
        json_content = buf.getvalue().decode("utf-8", errors="ignore")
    except Exception:
        logger.error("JSON download failed", exc_info=True)
        await msg.reply_text("❌ Could not read file.")
        return True

    questions, subject, total = parse_json_questions(json_content)
    if not questions:
        await msg.reply_text("❌ No questions found in this JSON file.")
        return True

    logger.info("[JSON IMPORT] '%s' %d questions", subject, len(questions))
    await set_admin_state(user_id, "awaiting_json_batch_number",
                          {"questions": questions, "subject": subject})
    await msg.reply_text(
        f"📋 *{esc(subject)}*\n\nFound *{len(questions)}* questions — all pre-marked.\n\n"
        f"⏳ Explanations will be generated smartly \\(extracted or AI fallback\\)\\.\n\nEnter a batch number:",
        parse_mode="MarkdownV2")
    return True

# ── HTML batch processor ──
async def process_html_batch(msg, user_id, context, state_data):
    ...
    for q in questions:
        q_id = await add_question(user_id, q["text"], q["options"],
                                   q["correct_index"], q.get("explanation", ""),
...
# ── JSON batch processor (with smart explanation) ──
async def process_json_batch(msg, user_id, context, state_data):
    text = msg.text.strip() if msg.text else ""
    if not text.isdigit():
        await msg.reply_text("Send a number. Or /cancel.")
        return True
    ...
    await msg.reply_text(f"⏳ Processing {len(questions)} questions with smart explanations...")

    added = 0
    for q in questions:
        explanation = await _smart_explanation(
            q.get("solution_raw", ""),
            q["text"], q["options"], q["correct_index"]
        )
        q_id = await add_question(user_id, q["text"], q["options"],
                                   q["correct_index"], explanation,
                                   photo_file_id=None, ai_source="json_import")
        added += 1
        logger.info("[QUESTION SAVED] QID=%d source=json_import explanation='%s'",
                    q_id, explanation[:40])
...
async def handle_question_forward(msg, user_id, context) -> bool:
    ...
    if msg.document:
        if msg.document.file_name.endswith(".json"):
            return await handle_json_upload(msg, user_id, context)
    ...
    if msg.poll:
        ...
        if poll.type == "quiz" and poll.correct_option_id is not None:
            ...
            q_id = await add_question(user_id, question_text, options,
                                       poll.correct_option_id, existing_explanation,
                                       photo_file_id, "telegram_quiz")
            logger.info("[QUESTION SAVED] QID=%d [AUTO-CONFIRMED] ANSWER=%s", q_id, chr(65+poll.correct_option_id))
            await msg.reply_text(
                f"✅ *Auto-confirmed* — Answer: *{chr(65+poll.correct_option_id)}*\n"
                f"Q: {esc(question_text[:100])}\n"
                f"{'📖 Explanation copied.' if existing_explanation else '_(No explanation in poll)_'}",
                parse_mode="Markdown", reply_markup=_make_confirm_kb(q_id))
            return True
